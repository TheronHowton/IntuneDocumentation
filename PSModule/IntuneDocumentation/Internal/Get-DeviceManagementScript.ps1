Function Get-DeviceManagementScript() {
    ...
    try {
        $uri = "https://graph.microsoft.us/Beta/deviceManagement/deviceManagementScripts"
        $request = (Invoke-MSGraphRequest -Url $uri -HttpMethod GET)
        $allScripts = @()

        $request.value.GetEnumerator() | ForEach-Object {

            $currentScript = Invoke-MSGraphRequest -HttpMethod GET -Url "https://graph.microsoft.us/Beta/deviceManagement/deviceManagementScripts/$($PSItem.id)"
